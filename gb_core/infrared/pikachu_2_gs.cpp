			...
			case RECEIVE_HELLO:
			{
				if (in_ir_signals.size() == 3)
				{
					if (got_hello_msg())
					{
						...
						set_sending_delay(micro_seconds_to_clocks(10000));
...
{
	out_ir_signals.push_back(new ir_signal(1, micro_seconds_to_clocks(126)));
	out_ir_signals.push_back(new ir_signal(0, micro_seconds_to_clocks(537)));
	out_ir_signals.push_back(new ir_signal(1, micro_seconds_to_clocks(126)));
...
{
	if (v_gb[0]->get_cpu()->get_clock() <= delay_start_clock) 
		is_waiting_for_delay_carry = false; 

	return is_waiting_for_delay_carry ? false : v_gb[0]->get_cpu()->get_clock() <= sending_delay;
}
